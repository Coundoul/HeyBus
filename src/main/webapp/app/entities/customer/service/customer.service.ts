...
export class CustomerService {
  ...
  protected convertDateFromClient(customer: ICustomer): ICustomer {
    return Object.assign({}, customer, {
      datenaissance: customer.datenaissance?.isValid() ? customer.datenaissance.format(DATE_FORMAT) : undefined,
      dateprisecontact: customer.dateprisecontact?.isValid() ? customer.dateprisecontact.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.dateprisecontact = res.body.dateprisecontact ? dayjs(res.body.dateprisecontact) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((customer: ICustomer) => {
        customer.datenaissance = customer.datenaissance ? dayjs(customer.datenaissance) : undefined;
        customer.dateprisecontact = customer.dateprisecontact ? dayjs(customer.dateprisecontact) : undefined;
      });
    }
    return res;
  }
