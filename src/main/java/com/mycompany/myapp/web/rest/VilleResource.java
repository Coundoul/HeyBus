...
public class VilleResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/villes/{id}", consumes = "application/merge-patch+json")
    ...
    public List<Ville> getAllVilles(@RequestParam(required = false) String filter) {
        if ("nomarret-is-null".equals(filter)) {
            log.debug("REST request to get all Villes where nomarret is null");
            return StreamSupport
                .stream(villeRepository.findAll().spliterator(), false)
                .filter(ville -> ville.getNomarret() == null)
